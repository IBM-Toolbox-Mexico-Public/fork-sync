...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            listTags.data.reverse().forEach((tag) => __awaiter(this, void 0, void 0, function* () {
                console.log(tag.name + ' sha1: ', tag.commit.sha);
                let getTag;
                try {
                    getTag = yield octokit.git.getTag({
                ...
                if (getTag === false) {
                    try {
                        createTag = yield octokit.git.createTag({
                            owner: context.repo.owner,
                            repo: context.repo.repo,
                            tag: tag.name,
                ...
                let createRelease;
                try {
                    createRelease = yield octokit.repos.createRelease({
                        owner: context.repo.owner,
                        repo: context.repo.repo,
                        tag_name: tag.name,
                    });
